...
static struct seg_tx {
	...
	u8_t               attempts;      /* Remaining tx attempts */
	u8_t               ttl;           /* Transmitted TTL value */
	u8_t               seg_pending;   /* Number of segments pending */
	u8_t               blocked:1,     /* Blocked by ongoing tx */
				 ...
				 struct net_buf_simple *sdu)
{
	...
	if (node == NULL) {
		/* Modified by bouffalolab*/
		BT_DBG("No node found for addr 0x%04x", rx->ctx.addr);
		return -EINVAL;
	}
...
static int app_key_decrypt(struct bt_mesh_net_rx *rx,
			   ...
			   u8_t hdr, u8_t aszmic, struct net_buf_simple *buf,
			   struct net_buf_simple *sdu)
{
...
void bt_mesh_rx_reset_node(u16_t src)
{
	int i;
	...
	bt_mesh_clear_node_rpl(src);
	bt_mesh_msg_cache_del(src);
}
...
void bt_mesh_trans_init(void)
{
	 ...
	 *K_MEM_SLAB_DEFINE(segs, BT_MESH_APP_SEG_SDU_MAX, CONFIG_BT_MESH_SEG_BUFS, 4);
	 */
	k_mem_slab_init(&segs, _k_mem_slab_buf_segs, BT_MESH_APP_SEG_SDU_MAX, CONFIG_BT_MESH_SEG_BUFS);
	...
	if (app_idx == BT_MESH_KEY_DEV_LOCAL ||
		...
		*key = bt_mesh.dev_key;
		...
		if (!IS_ENABLED(CONFIG_BT_MESH_CDB)) {
			/** Modified by bouffalo 
			 * When disable cdb, use device key.
			 */
			*aid = 0;
			*key = bt_mesh.dev_key;
		...
		if (!node) {
			/** Modified by bouffalo 
			 * When no addr found, use device key.
			 */
			*aid = 0;
			*key = bt_mesh.dev_key;
