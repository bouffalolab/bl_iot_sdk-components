...
static uint8_t tx_buf[2048], rx_buf[2048];
static volatile uint32_t cnt = 0;
static SemaphoreHandle_t xSemaphore;

static __attribute__((section(".tcm_code.lramsync"))) void _ramsync_rx(void *arg)
{
...
static __attribute__((section(".tcm_code.lramsync"))) void _ramsync_tx(void *arg)
{
    //printf("tx handler\r\n");
}

static void cmd_lrmsync_reset(char *buf, int len, int argc, char **argv)
{
	lramsync_reset(&ramsync_ctx);
}
...
int lramsync_test(void)
{
    ...
    tx[0].buf = tx_buf;
    tx[0].len = sizeof(tx_buf) / 2;
    tx[1].buf = &tx_buf[tx[0].len];
    tx[1].len = sizeof(tx_buf) / 2;

    rx[0].buf = rx_buf;
    rx[0].len = sizeof(rx_buf) / 2;
    rx[1].buf = &rx_buf[rx[0].len];
    rx[1].len = sizeof(rx_buf) / 2;
    ...
    while (1) {
    	...
    	if (cnt_tmp < cnt) {
    		...
    		for (int i = 0; i < sizeof(rx_buf) / 2; i++) {
    			if (rx_buf[offset + i] != (i & 0xff)) {
    				printf("rx failed!!!!!\r\n");
    				break;
    			}
    		}
    		//printf("rx ok %ld\r\n", cnt_tmp);
    		//tpdbg_buf("rx", rx_buf, 16);
    		cnt_tmp++;
    	}
    }
    return 0;
...
const static struct cli_command cmds_user[] STATIC_CLI_CMD_ATTRIBUTE = {
        ...
        { "lrmsync_reset", "lrmsync_reset", cmd_lrmsync_reset},
};
