...
*/
const uint32_t utils_crc_crc32_tab[256] = {
...
uint32_t utils_crc32_accumulate(uint32_t crc, const void *dataIn, uint32_t len)
{
    ...
    while (len--)
        crc = utils_crc_crc32_tab[(crc ^ *data++) & 0xFF] ^ (crc >> 8);
...
{
    return utils_crc32_accumulate(0, dataIn, len);
}
...
{
    ctx->crc = utils_crc_crc32_tab[(ctx->crc ^ data) & 0xFF] ^ (ctx->crc >> 8);
}
