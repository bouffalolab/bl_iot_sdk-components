...
int eth_phy_scan(eth_phy_ctrl_t *phy_ctrl, uint8_t start_addr, uint8_t end_addr)
{
    ...
    for (phy_addr = start_addr; phy_addr <= end_addr; phy_addr++) {
        ...
        if (phy_id != 0 && phy_id != 0xffffffff) {
            log_info("eth phy scan success, phy_addr: %d, phy_id: 0x%08X\r\n", phy_addr, phy_id);
            break;
        }
    }

    if (phy_addr > end_addr) {
        log_error("eth phy scan failed\r\n");
        return -1;
    }
...
int eth_phy_init(eth_phy_ctrl_t *phy_ctrl, eth_phy_init_cfg_t *cfg)
{
    ...
    if (phy_ctrl->phy_drv == NULL) {
        log_info("no phy_drv, try to match\r\n");
        if (eth_phy_drv_match(phy_ctrl) < 0) {
            log_error("eth phy drv_match failed\r\n");
            return -1;
        } else {
            log_info("eth phy drv_match success, phy_name: %s\r\n", phy_ctrl->phy_drv->phy_name);
        }
    }
...
int eth_phy_ctrl(eth_phy_ctrl_t *phy_ctrl, uint32_t cmd, uint32_t arg)
{
    ...
    if (phy_ctrl->phy_drv->phy_ctrl) {
        ...
        log_error("no phy_ctrl\r\n");
        return -1;
    }
}

int eth_phy_dump_registers(struct eth_phy_ctrl_s *phy_ctrl)
{
    if (phy_ctrl->phy_drv == NULL) {
        log_error("no phy_drv\r\n");
        return -1;
    ...
    if (phy_ctrl->phy_drv->dump_registers) {
        return phy_ctrl->phy_drv->dump_registers(phy_ctrl);
    } else {
        log_error("no dump_registers\r\n");
        return -1;
    }
}
