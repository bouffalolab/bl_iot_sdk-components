...
static void board_pin_mux_init(void)
{
    ...
    for (int i = 0; i < sizeof(af_pin_table) / sizeof(af_pin_table[0]); i++) {
        ...
        if (af_pin_table[i].func == GPIO_FUN_UNUSED) {
            continue;
        } else if (af_pin_table[i].func == GPIO_FUN_ADC) {
...
void system_clock_init(void)
{
    ...
    GLB_Set_MTimer_CLK(1, GLB_MTIMER_CLK_XCLK, Clock_System_Clock_Get(BL_SYSTEM_CLOCK_XCLK) / 1000 / 1000 - 1);
...
#error "please select correct uart clock source"
    ...
    tmpVal |= (1 << BL_AHB_SLAVE1_I2C);
    GLB_Set_I2C_CLK(ENABLE, GLB_I2C_CLK_SRC_BCLK, BSP_I2C_CLOCK_DIV);
#elif BSP_I2C_CLOCK_SOURCE == ROOT_CLOCK_SOURCE_XCLK
    tmpVal |= (1 << BL_AHB_SLAVE1_I2C);
    GLB_Set_I2C_CLK(ENABLE, GLB_I2C_CLK_SRC_XCLK, BSP_I2C_CLOCK_DIV);
#else
#error "please select correct i2c clock source"
    ...
    tmpVal |= (1 << BL_AHB_SLAVE1_SPI);
    GLB_Set_SPI_CLK(ENABLE, GLB_SPI_CLK_SRC_BCLK, BSP_SPI_CLOCK_DIV);
#elif BSP_SPI_CLOCK_SOURCE == ROOT_CLOCK_SOURCE_XCLK
    tmpVal |= (1 << BL_AHB_SLAVE1_SPI);
    GLB_Set_SPI_CLK(ENABLE, GLB_SPI_CLK_SRC_XCLK, BSP_SPI_CLOCK_DIV);
#else
#error "please select correct spi clock source"
...
#if BSP_TIMER0_CLOCK_SOURCE == ROOT_CLOCK_SOURCE_XCLK
    /* Configure timer clock source */
    uint32_t tmp = BL_RD_REG(TIMER_BASE, TIMER_TCCR);
    tmp = BL_SET_REG_BITS_VAL(tmp, TIMER_CS_2, TIMER_CLKSRC_XTAL);
    ...
    /* Configure timer clock source */
    uint32_t tmp = BL_RD_REG(TIMER_BASE, TIMER_TCCR);
    tmp = BL_SET_REG_BITS_VAL(tmp, TIMER_CS_2, TIMER_CLKSRC_32K);
    BL_WR_REG(TIMER_BASE, TIMER_TCCR, tmp);
    ...
    /* Configure timer clock source */
    uint32_t tmp = BL_RD_REG(TIMER_BASE, TIMER_TCCR);
    tmp = BL_SET_REG_BITS_VAL(tmp, TIMER_CS_2, TIMER_CLKSRC_1K);
    BL_WR_REG(TIMER_BASE, TIMER_TCCR, tmp);
...
#error "please select correct timer0 clock source"
...
#if BSP_TIMER1_CLOCK_SOURCE == ROOT_CLOCK_SOURCE_XCLK
    /* Configure timer clock source */
    uint32_t tmp1 = BL_RD_REG(TIMER_BASE, TIMER_TCCR);
    tmp1 = BL_SET_REG_BITS_VAL(tmp1, TIMER_CS_3, TIMER_CLKSRC_XTAL);
    ...
    /* Configure timer clock source */
    uint32_t tmp1 = BL_RD_REG(TIMER_BASE, TIMER_TCCR);
    tmp1 = BL_SET_REG_BITS_VAL(tmp1, TIMER_CS_3, TIMER_CLKSRC_32K);
    BL_WR_REG(TIMER_BASE, TIMER_TCCR, tmp1);
    ...
    /* Configure timer clock source */
    uint32_t tmp1 = BL_RD_REG(TIMER_BASE, TIMER_TCCR);
    tmp1 = BL_SET_REG_BITS_VAL(tmp1, TIMER_CS_3, TIMER_CLKSRC_1K);
    BL_WR_REG(TIMER_BASE, TIMER_TCCR, tmp1);
...
#if BSP_WDT_CLOCK_SOURCE == ROOT_CLOCK_SOURCE_XCLK
...
#error "please select correct watchdog timer clock source"
    ...
    tmpVal |= (1 << BL_AHB_SLAVE1_GPIP);
#if BSP_ADC_CLOCK_SOURCE == ROOT_CLOCK_SOURCE_32K_CLK
    GLB_Set_ADC_CLK(ENABLE, GLB_ADC_CLK_SRC_F32K, BSP_ADC_CLOCK_DIV);
#elif BSP_ADC_CLOCK_SOURCE == ROOT_CLOCK_SOURCE_XCLK
    GLB_Set_ADC_CLK(ENABLE, GLB_ADC_CLK_SRC_XCLK, BSP_ADC_CLOCK_DIV);
#else
...
void rf_reset_done_callback(void)
{
    ...
    rf_set_bz_mode(2/*MODE_BZ_COEX*/);
#else
    rf_set_bz_mode(0/*MODE_BLE_ONLY*/);
#endif
#else
    rf_set_bz_mode(1/*MODE_ZB_ONLY*/);
#endif
}
