...
int sec_dsa_mexp_binary(uint32_t size, const uint32_t *a, const uint32_t *b, const uint32_t *c, uint32_t *r)
{
    ...
    MSG("number:\r\n");
    bflb_platform_dump((uint8_t*)pka_tmp, dataSize * 4);
#endif
    ...
    for (; i >= k; i--) {
        ...
        for (j = 0; j < 8; j++) {
            ...
            if (isOne) {
                ...
                Sec_Eng_PKA_Read_Data(nregType, 5, (uint32_t *)pka_tmp, dataSize);
                MSG("number:\r\n");
                bflb_platform_dump((uint8_t*)pka_tmp, dataSize /*dataSize*4*/);
#endif
            }
...
#ifdef DSA_DBG
    MSG("r:\r\n");
    bflb_platform_dump((uint8_t*)r, dataSize * 4);
#endif
    return 0;
}
...
#ifdef DSA_DBG
    MSG("r:\r\n");
    bflb_platform_dump((uint8_t*)r, dataSize /*dataSize*4*/);
#endif
    return 0;
}
...
int sec_dsa_decrypt_crt(uint32_t size, uint32_t *c, sec_dsa_crt_cfg_t *crtCfg, uint32_t *d, uint32_t *r)
{
    ...
    MSG("qInv * (m1 - m2):\r\n");
    bflb_platform_dump((uint8_t*)pka_tmp, dataSize /*dataSize*4*2*/);
#endif
...
#ifdef DSA_DBG
    MSG("r:\r\n");
    bflb_platform_dump((uint8_t*)r, dataSize * 4 * 2);
#endif
    return 0;
}
