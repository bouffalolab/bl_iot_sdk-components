...
int spi_open(struct device *dev, uint16_t oflag)
{
    ...
    GLB_Swap_SPI_0_MOSI_With_MISO(spi_device->pin_swap_enable);
...
#ifdef BSP_USING_SPI0
        if (spi_device->id == 0) {
            Interrupt_Handler_Register(SPI_IRQn, SPI0_IRQ_Function);
        }
#endif
    }
...
int spi_write(struct device *dev, uint32_t pos, const void *buffer, uint32_t size)
{
    ...
    if (dev->oflag & DEVICE_OFLAG_DMA_TX) {
        ...
        ret = dma_reload(dma_ch, (uint32_t)(uintptr_t)buffer, (uint32_t)DMA_ADDR_SPI_TDR, size);
...
int spi_read(struct device *dev, uint32_t pos, void *buffer, uint32_t size)
{
    ...
    if (dev->oflag & DEVICE_OFLAG_DMA_RX) {
        ...
        ret = dma_reload(dma_ch, (uint32_t)DMA_ADDR_SPI_RDR, (uint32_t)(uintptr_t)buffer, size);
        dma_channel_start(dma_ch);
        return ret;
