...
{
    uint8_t nlBits = *(volatile uint8_t *)(CLIC_HART0_ADDR + CLIC_CFG) >> 1 & 0xf;
    uint8_t clicIntCfg = *(volatile uint8_t *)(CLIC_HART0_ADDR + CLIC_INTCFG + IRQn);
    *(volatile uint8_t *)(CLIC_HART0_ADDR + CLIC_INTCFG + IRQn) = (clicIntCfg & 0xf) | (PreemptPriority << (8 - nlBits)) | ((SubPriority & (0xf >> nlBits)) << 4);
}
...
void clic_set_nlBits(uint32_t nlBits)
{
    uint8_t clicCfg = *(volatile uint8_t *)(CLIC_HART0_ADDR + CLIC_CFG);
    *(volatile uint8_t *)(CLIC_HART0_ADDR + CLIC_CFG) = (clicCfg & 0xe1) | ((nlBits & 0xf) << 1);
}
