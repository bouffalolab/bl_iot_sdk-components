...
static isp_awb_stats_info_t awb_stats_info;
static isp_awb_sum_t awb_stats_data[AWB_STATS_GRID_NUM];
...
static void awb_stats_reset()
{
    ...
    memset(awb_stats_info.awb_stats_data, 0, AWB_STATS_GRID_NUM*sizeof(isp_awb_sum_t));
    awb_stats_info.is_awb_stats_update = -1;
}

static void rgb_sum_to_wb_ratio(RGB_SUM_S *rgb_sum, ISP_RB_Gain_Type *rb_gain, wb_ratio_t *wb_ratio)
{
    /* (g_sum / r_sum) * 1024 */
...
{
	int wr_idx;
    int wr_num = white_region_use.num_white_region;
    const white_region_t *white_region;

    for (wr_idx = 0; wr_idx < wr_num; wr_idx++) {
		white_region = &white_region_use.white_region[wr_idx];
...
static int weight_avg_color_temp(int white_point_cnt[], BL_COLOR_TEMP *color_temp)
{
    int i;
    const white_region_t *white_region;
    int num_regions = white_region_use.num_white_region;
    int ct_sum = 0;
    int total_wp_cnt = 0;

    for (i = 0; i < num_regions; i++) {
        white_region = &white_region_use.white_region[i];
...
static int calc_color_temp(ISP_RB_Gain_Type *rb_gain, BL_COLOR_TEMP *color_temp)
{
	int i;
    int valid_grid_num;
    RGB_SUM_S rgb_sum;
    wb_ratio_t wb_ratio;
    int white_point_cnt[white_region_use.num_white_region];

    BL_LOGD("curr rb gain (%d, %d)\r\n", rb_gain->r_gain, rb_gain->b_gain);
    memset(white_point_cnt, 0, sizeof(white_point_cnt));

    ISP_Get_AWB_ValidGridNum(&valid_grid_num);

	for (i = 0; i < AWB_STATS_GRID_NUM; i++) {
        // ISP_Get_AWB_Sum(i, &rgb_sum);
        rgb_sum.r_sum = ((isp_awb_sum_t*)(awb_stats_info.awb_stats_data))[i].r_sum;
        rgb_sum.g_sum = ((isp_awb_sum_t*)(awb_stats_info.awb_stats_data))[i].g_sum;
        rgb_sum.b_sum = ((isp_awb_sum_t*)(awb_stats_info.awb_stats_data))[i].b_sum;
//        BL_LOGI("rgb_sum[%d] (%d, %d, %d)\r\n", i, rgb_sum.r_sum, rgb_sum.g_sum, rgb_sum.b_sum);

        rgb_sum_to_wb_ratio(&rgb_sum, rb_gain, &wb_ratio);

//        BL_LOGI("\t tile (%d, %d), wb ratio (%d, %d)\r\n", i/32, i%32, wb_ratio.gr_ratio, wb_ratio.gb_ratio);
//		BL_LOGI("\t %d_%d \r\n", wb_ratio.gr_ratio, wb_ratio.gb_ratio); //for AWB tuning with AMB_AWB_tool
        match_all_white_region(&wb_ratio, white_point_cnt);
	}

    BL_LOGD("white point count:\r\n");

    weight_avg_color_temp(white_point_cnt, color_temp);

    BL_LOGD(">> target CT: %dK\r\n", *color_temp);
...
static void set_rgb_threshold(isp_awb_stats_conf_t *awb_stats_conf)
{
#if 0
...
{
    isp_awb_stats_conf_t awb_stats_conf;
    ...
    ISP_Set_AWB_Stats_Conf(&awb_stats_conf);
    ISP_Set_AWB_State(ENABLE);
...
static int get_rgb_sum(RGB_SUM_S *rgb_sum)
{
    ...
    ISP_Get_AWB_ValidGridNum(&total_grid_num, &valid_grid_num);

	for (i = 0; i < total_grid_num; i++) {
        ISP_Get_AWB_Sum(i, &grid_rgb_sum, &buf_idx);
        ...
        if (buf_idx != init_buf_idx) {
            BL_LOGI("Got AWB stats from different frames! %d, %d\r\n", init_buf_idx, buf_idx);
        }
//        BL_LOGI("\t\t grid[%d]: r %d, g %d, b %d\r\n", i, grid_rgb_sum.r_sum, grid_rgb_sum.g_sum, grid_rgb_sum.b_sum);
	}


//    BL_LOGD("valid grid number: %d\r\n", valid_grid_num);
//    BL_LOGD("Rsum %d, Gsum %d, Bsum %d\r\n", rgb_sum->r_sum, rgb_sum->g_sum, rgb_sum->b_sum);

	return 0;
}
#endif

static int calc_rb_gain_by_color_temp(BL_COLOR_TEMP color_temp, ISP_RB_Gain_Type *rb_gain)
{
    int wb_info_idx;
    ...
    int gain_tmp;
    int num_wb_modes = wb_info_use.num_wb_info;

    for (wb_info_idx = 0; wb_info_idx < (num_wb_modes - 1); wb_info_idx++) {

		wb_info_L = &wb_info_use.wb_info[wb_info_idx];
		wb_info_H = &wb_info_use.wb_info[wb_info_idx + 1];
...
static int awb1_grey_world(awb_obj_t *awb_obj)
{
    ...
    BL_LOGD("Rsum %d, Gsum %d, Bsum %d\r\n", rgb_sum.r_sum, rgb_sum.g_sum, rgb_sum.b_sum);
    BL_LOGD("\t R/G ratio = %.4f, B/G ratio = %.4f\r\n",
	...
	BL_LOGD("\t Rerr = %.4f, Berr = %.4f\r\n", Rerr, Berr);

	rgain_f = (rgain_f + Rerr*K);
	bgain_f = (bgain_f + Berr*K);

    BL_LOGD("rgain_f %.2f, bgain_f %.2f\r\n", rgain_f, bgain_f);
...
static void awb1_update_stats(void)
{
    ...
    awb_stats_info.is_awb_stats_update = ISP_Get_AWB_Stats(AWB_STATS_GRID_NUM, \
...
static int awb1_set_mode(AWB_MODE mode)
{
    if (!is_param_valid(mode, AWB_MODE_MIN, AWB_MODE_MAX)) {
        BL_LOGE("awb_set_mode: invalid AWB mode %d!\r\n", mode);
        return -1;
    }
...
static int awb1_set_color_temp(BL_COLOR_TEMP color_temp)
{
    ...
    BL_LOGI("awb_set_color_temp: %d K\r\n", color_temp);
...
static int awb1_set_gain(ISP_RGB_Gain_Type *rgb_gain)
{
...
static int awb1_get_gain(ISP_RGB_Gain_Type *rgb_gain)
{
...
static int awb1_control(void)
{
    if (!awb_obj.awb_configed) {
        BL_LOGD("AWB can't run! Not configured\r\n");
        return -1;
    }
...
int awb1_init(awb_func_t *func)
{
    ...
    ISP_Set_RB_Gain(&(awb_obj.awb_info.rb_gain));

    awb_func_init(func);

    BL_LOGD("awb_config: rgain %d, bgain %d\r\n", awb_obj.awb_info.rb_gain.r_gain, awb_obj.awb_info.rb_gain.b_gain);
    return 0;
}