...
typedef struct
{
    ...
    ISP_WDR_Hist_Data_Type hist; // r, ping-pong, 24'b, = HIST_BW
...
static wdr_param_sw_t     wdr_sw = {0};
static int flag_sw_init = 0;
...
#else
    config->param.cs_weight = wdr_fw_config.param.cs_weight;
    config->param.cs_gain_thresh = wdr_fw_config.param.cs_gain_thresh;
    config->param.curve_weight = wdr_fw_config.param.curve_weight;
    config->param.enh_weight = wdr_fw_config.param.enh_weight;
    config->param.enh_c_gain_extra_slope = wdr_fw_config.param.enh_c_gain_extra_slope;
#endif
}
...
{
    ISP_WDR_Set_CS_State(ENABLE);
    ISP_WDR_Set_WDR_Curve_State(ENABLE);
    ISP_WDR_Set_Post_Enhance_State(ENABLE);
}
...
void wdr_hw_update(void)
{
    ...
    ISP_Chroma_Suppress_Type cs_cfg;
    // apply wdr curve
    for (i = 0; i < ISP_WDR_CURVE_COEF_NUM; i++) {
    ...
    ISP_WDR_Set_CS_Config(&cs_cfg);
    ISP_WDR_Set_WDR_Curve(&wdr_curve);
    ISP_WDR_Set_WDR_Curve_Weight(wdr_reg.curve_weight);
    ISP_WDR_Set_Post_Enhance_Weight(wdr_reg.enh_weight);
    ISP_WDR_Set_Post_Enhance_Y_Offset(wdr_reg.enh_y_offset);
    ISP_WDR_Set_Post_Enhance_Luma_Gain(wdr_reg.enh_y_gain);
    ISP_WDR_Set_Post_Enhance_Chroma_Gain(wdr_reg.enh_c_gain);
}

int wdr_control(const wdr_fw_config_t *config)
{

    memcpy(&wdr_fw, config, sizeof(wdr_fw_config_t));

    // run after each frame hardware done (WDR interrupt)
    wdr_decide_chroma_suppression(&wdr_fw);
    wdr_hist_pre_process();
    wdr_decide_curve();
    wdr_decide_enhancement();
    wdr_decide_fallback(&wdr_fw);
    wdr_temporal_smooth();
...
int wdr_init()
{
    //only no sw param, when get sw param, then set sw param
    flag_sw_init = 1;
...
int wdr_sw_get_fw_register(wdr_param_sw_t *sw, wdr_fw_config_t *fw)
{
	...
	trans_tbl = (wdr_trans_tbl_t *)&wdr_trans_use.cs_strength_tbl;
	tbl_num = (int)sizeof(trans_tbl) / (int)sizeof(trans_tbl[0]);

    wdr_get_fw_config(fw);
    table_linear_interpolate(trans_tbl->cs_strength_tbl, trans_tbl->cs_weight_tbl, tbl_num, sw->cs_strength, &fw->param.cs_weight);
    table_linear_interpolate(trans_tbl->cs_strength_tbl, trans_tbl->cs_gain_thresh_tbl, tbl_num, sw->cs_strength, &fw->param.cs_gain_thresh);
    
    fw->param.curve_weight = sw->curve_weight;
    fw->param.enh_weight = sw->enh_weight;
    fw->sys_gain_db = sw->sys_gain_db;
...
int wdr_fw_set_sw_register(wdr_fw_config_t *fw)
{
    ...
    table_linear_interpolate(trans_tbl->cs_weight_tbl, trans_tbl->cs_strength_tbl, tbl_num, fw->param.cs_weight, &wdr_sw.cs_strength);
    wdr_sw.curve_weight = fw->param.curve_weight;
    wdr_sw.enh_weight = fw->param.enh_weight;
    wdr_sw.sys_gain_db = fw->sys_gain_db;
...
void wdr_get_sw_config(wdr_param_sw_t *config)
{
    ...
    if (flag_sw_init == 0) {//only no sw param, when get sw param, then set sw param
        flag_sw_init = 1;
        table_linear_interpolate(trans_tbl->cs_weight_tbl, trans_tbl->cs_strength_tbl, tbl_num, wdr_fw.param.cs_weight, &wdr_sw.cs_strength);
        wdr_sw.curve_weight = wdr_fw.param.curve_weight;
        wdr_sw.enh_weight = wdr_fw.param.enh_weight;
        wdr_sw.sys_gain_db = wdr_fw.sys_gain_db;
    }

    config->cs_strength = wdr_sw.cs_strength;
    config->curve_weight = wdr_sw.curve_weight;
    config->enh_weight = wdr_sw.enh_weight;
    config->sys_gain_db = wdr_sw.sys_gain_db;
}
...
void wdr_set_sw_config(wdr_param_sw_t *config)
{
    ...
    wdr_sw.sys_gain_db = config->sys_gain_db;
}
...
int wdr_config(wdr_config_t *config, bool is_virtual_sensor)
{
    ...
    ISP_WDR_Set_ROI_State(0, ENABLE);
    ISP_WDR_Set_ROI_Weight(0, 1);
    ISP_WDR_Set_ROI_Position(0, 0, config->stats_cfg.act_win.width-1, 0, config->stats_cfg.act_win.height-1);
...
static void wdr_hist_pre_process()
{
    ISP_WDR_Get_Hist_Data(&wdr_reg.hist);
...
static void wdr_temporal_smooth()
{
    ...
    BL_LOGD("cs_grey_th %d, cs_gain %d , cs_weight%d , curve_weight %d, enh_weight %d , enh_y_offset %d , enh_y_gain %d\r\n", \
