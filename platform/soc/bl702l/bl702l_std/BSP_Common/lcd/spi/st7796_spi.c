    ...
    switch (dir) {
        case 0:
            if (!mir_flag)
                ...
                st7796_spi_write_data_byte(0x48);
            break;
        case 1:
            if (!mir_flag)
                st7796_spi_write_data_byte(0xA8);
            else
                st7796_spi_write_data_byte(0x28);
            break;
        case 2:
            if (!mir_flag)
                st7796_spi_write_data_byte(0xC8);
            else
                st7796_spi_write_data_byte(0x88);
            break;
        case 3:
            if (!mir_flag)
                st7796_spi_write_data_byte(0x68);
            else
...
void st7796_spi_draw_picture_nonblocking(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, st7796_spi_color_t *picture)
{
    ...
    device_control(st7796_spi, DEVICE_CTRL_TX_DMA_RESUME, NULL);
    ret = device_write(st7796_spi, 0, picture, (picture_size<<1));
    if (ret != 0) {
