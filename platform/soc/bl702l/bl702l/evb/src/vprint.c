	   ...
	   int base, int width, int prec)
{
	...
	o += ndigits;
	char qqbuf[16]; qq = qqbuf + sizeof(qqbuf);//qq = q;
...
static char *flt(char *str, double num, int size, int precision, char fmt, int flags)
{
  ...
  size -= n;
  if (!(flags & (ZEROPAD | LEFT))) while (size-- > 0) OUT(str, ' ');//*str++ = ' ';
  if (sign) OUT(str, sign);//*str++ = sign;
  if (!(flags & LEFT)) while (size-- > 0) OUT(str, c);//*str++ = c;
  for (i = 0; i < n; i++) OUT(str, tmp[i]);//*str++ = tmp[i];
  while (size-- > 0) OUT(str, ' ');//*str++ = ' ';
