...
static void gpio_init(uint8_t id, uint8_t tx_pin, uint8_t rx_pin, uint8_t cts_pin, uint8_t rts_pin)
{
    ...
    GLB_UART_Fun_Sel(tx_pin%4, tx_sigfun);
    GLB_UART_Fun_Sel(rx_pin%4, rx_sigfun);
}
...
static int __uart_dma_txcfg(hosal_uart_dev_t *uart, hosal_uart_dma_cfg_t *dma_cfg)
{
    ...
    if (uart->dma_tx_chan >= 0) {
    	DMA_Channel_Update_SrcMemcfg(DMA0_ID, uart->dma_tx_chan,
...
static int __uart_dma_rxcfg(hosal_uart_dev_t *uart, hosal_uart_dma_cfg_t *dma_cfg)
{
    ...
    if (uart->dma_rx_chan >= 0) {
    	DMA_Channel_Update_DstMemcfg(DMA0_ID, uart->dma_rx_chan,
    			(uint32_t)dma_cfg->dma_buf, dma_cfg->dma_buf_size);
    	return 0;
    }
...
static void __uart_config_set(hosal_uart_dev_t *uart, const hosal_uart_config_t *cfg)
{
    ...
    uartCfg.uartClk = 32000000 / (uart_div + 1);
...
{
    hosal_uart_dma_cfg_t rxdma_cfg = {
        .dma_buf = uart_dma_rx_buf,
...
int hosal_uart_dma_rx_start(hosal_uart_dev_t *uart)
{
    if (uart->dma_rx_chan < 0) {
        return -1;
    }

    DMA_Channel_Disable(DMA0_ID, uart->dma_rx_chan);
    DMA_Channel_Update_DstMemcfg(DMA0_ID, uart->dma_rx_chan, (uint32_t)uart_dma_rx_buf, sizeof(uart_dma_rx_buf));
    DMA_Channel_Enable(DMA0_ID, uart->dma_rx_chan);
...
int hosal_uart_dma_rx_get_data(hosal_uart_dev_t *uart, uint8_t *buf, uint32_t buf_size)
{
    ...
    if (len > 0) {
        ...
        memcpy(buf, uart_dma_rx_buf, len);
        return len;
    }
