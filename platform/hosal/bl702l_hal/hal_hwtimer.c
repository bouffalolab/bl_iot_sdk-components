...
static void hwtimer_process(void *ctx)
{
    ...
    TIMER_ClearIntStatus(TIMER0_ID, HW_TIMER_CHANNEL, TIMER_COMP_ID_0);
    ...
    TIMER_IntMask(TIMER0_ID, HW_TIMER_CHANNEL, TIMER_INT_COMP_0, UNMASK);
}
...
//    GLB_AHB_Slave1_Reset(BL_AHB_SLAVE1_TMR);  // will do reset in bl_sys_early_init() in bl_sys.c
    TIMER_IntMask(TIMER0_ID, hw_timercfg.timerCh,TIMER_INT_ALL, MASK);
    TIMER_Disable(TIMER0_ID, hw_timercfg.timerCh);
    TIMER_Init(TIMER0_ID, &hw_timercfg);
    ...
    TIMER_Enable(TIMER0_ID, HW_TIMER_CHANNEL);
