...
#define DTCM_ADDR_START     0X201C000
#define DTCM_ADDR_END       (DTCM_ADDR_START + (16 * 1024)) 
#define OCRAM_ADDR_START    0X2020000
#define OCRAM_ADDR_END      (OCRAM_ADDR_START + (80 * 1024))

#define AVAIBLE_ADDR_START    DTCM_ADDR_START
#define AVAIBLE_ADDR_END      OCRAM_ADDR_END
...
static void inline _dma_copy_trigger(struct bl_dma_item *first)
{
    ...
    DMA_LLI_Update(DMA0_ID, DMA_DEFAULT_CHANNEL_COPY, (uint32_t)&(first->src));
    DMA_Channel_Enable(DMA0_ID, DMA_DEFAULT_CHANNEL_COPY);
}
...
void bl_dma_init(void)
{
    ...
    DMA_Enable(DMA0_ID);
    DMA_IntMask(DMA0_ID, dmaCh, DMA_INT_ALL, MASK);
    DMA_IntMask(DMA0_ID, dmaCh, DMA_INT_TCOMPLETED, UNMASK);
    DMA_IntMask(DMA0_ID, dmaCh, DMA_INT_ERR, UNMASK);
    DMA_LLI_Init(DMA0_ID, dmaCh, &lliCfg);
