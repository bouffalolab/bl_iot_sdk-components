...
static void tsen_dma_callback(void *p_arg, uint32_t flag)
{
    ...
    switch(tsen_fsm)
    {
        case 1:
            ...
            DMA_Channel_Update_DstMemcfg(DMA0_ID, tsen_dma_ch, (uint32_t)tsen_buf, sizeof(tsen_buf)/sizeof(tsen_buf[0]));
            DMA_Channel_Enable(DMA0_ID, tsen_dma_ch);
...
static void voice_dma_callback(void *p_arg, uint32_t flag)
{
    ...
    DMA_Channel_Update_DstMemcfg(DMA0_ID, adc_dma_ch, (uint32_t)pcm_frame_buf[pcm_frame_idx], pcm_frame_size);
    DMA_Channel_Enable(DMA0_ID, adc_dma_ch);
...
int bl_adc_voice_stop(void)
{
    ...
    DMA_Channel_Disable(DMA0_ID, adc_dma_ch);
