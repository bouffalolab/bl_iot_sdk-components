...
static int hosal_spi_dma_trans(hosal_spi_dev_t *spi, uint8_t *TxData, uint8_t *RxData, uint32_t Len, uint32_t timeout)
{
    ...
    if (RxData) {
        ...
        DMA_LLI_Init(DMA0_ID, dma_arg->rx_dma_ch, &rxllicfg);
        DMA_LLI_Update(DMA0_ID, dma_arg->rx_dma_ch, (uint32_t)prxlli);
        hosal_dma_chan_start(dma_arg->rx_dma_ch);
    }

    if (TxData) {
        hosal_dma_irq_callback_set(dma_arg->tx_dma_ch, hosal_spi_int_handler_tx, spi);
        DMA_LLI_Init(DMA0_ID, dma_arg->tx_dma_ch, &txllicfg);
        DMA_LLI_Update(DMA0_ID, dma_arg->tx_dma_ch, (uint32_t)ptxlli);
