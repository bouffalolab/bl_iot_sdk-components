                                                ...
                                                PtTable_Entry_Config *ptEntry)
{
    ...
    if(targetTableID==PT_TABLE_ID_0){
        ...
        /* Update address of partition table 0 from bootheader (offset = 164) */
        bl_flash_read(164, (uint8_t *)&writeAddr, 4);
    }else{
        writeAddr=BFLB_PT_TABLE1_ADDRESS;
        /* Update address of partition table 1 from bootheader (offset = 168) */
        bl_flash_read(168, (uint8_t *)&writeAddr, 4);
    }
