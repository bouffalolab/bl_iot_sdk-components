                           ...
                           unsigned char *b_input)
{
    ...
    */
    ...
    BL702_MemCpy_Fast( b + 1, iv, iv_len );
    ...
    BL702_MemCpy_Fast(b_input, b, 16);
    ...
    if( add_len > 0 )
    {
        ...
        */
        ...
        BL702_MemCpy_Fast(b + 2, src, use_len );
        len_left -= use_len;
        src += use_len;

        BL702_MemCpy_Fast(b_input + cbc_adata_round * 16, b, 16 );
        ...
        while( len_left > 0 )
        {
            ...
            if (use_len < 16) {
                BL702_MemSet(b_input + cbc_adata_round * 16, 0, 16);
            }

            BL702_MemCpy_Fast(b_input + cbc_adata_round * 16, src, use_len);
    ...
    while(len_left > 0)
    {
        ...
        BL702_MemCpy_Fast(b_input + cbc_pt_round * 16, src, use_len);
...
static int get_ctr_enc_input(size_t length, const unsigned char *input, unsigned char *b_input, unsigned char *b_output, unsigned int  cbc_pt_round)
{
    ...
    BL702_MemCpy_Fast(b_input, b_output + (cbc_pt_round - 1) * 16, 16);
    ...
    while( len_left > 0 )
    {
        ...
        BL702_MemCpy_Fast(b_input + ctr_pt_round * 16, src, use_len);
...
static int get_ctr_dec_input(size_t length, const unsigned char *input, const unsigned char *tag, size_t tag_len, unsigned char *b_input)
{
    ...
    BL702_MemCpy_Fast(b_input, tag, tag_len);
    ...
    while( len_left > 0 )
    {
        ...
        BL702_MemCpy_Fast(b_input + ctr_ct_round * 16, src, use_len);
...
static int get_ctr_iv(unsigned char *ctr_iv, const unsigned char *iv, size_t iv_len)
{
    ...
    q = 16 - 1 - (unsigned char) iv_len;
    ctr_iv[0] |= q - 1;
    BL702_MemCpy_Fast(ctr_iv + 1, iv, iv_len);
                         ...
                         const unsigned char *input, unsigned char *output, unsigned char *tag, size_t tag_len)
{
	...
	BL702_MemSet(b_input, 0, b_input_size);
	BL702_MemSet(b_output, 0, b_output_size);
	...
	BL702_MemCpy_Fast(tag, b_output, tag_len);

	/*second ~ ? output block is pt info*/
	BL702_MemCpy_Fast(output, b_output + 16, length);
...
exit:
							 ...
							 size_t add_len, const unsigned char *input, unsigned char *output, const unsigned char *tag, size_t tag_len)
 {
	...
	if(!b_output)
	{
	 ...
	 /* Copy CBC-MAC here */
	 BL702_MemCpy_Fast(cbc_mac, b_output, tag_len);
	 ...
	 /* Check CBC-MAC */
	 if (BL702_MemCmp(cbc_mac, b_output + (cbc_length/16 - 1) * 16, tag_len) != 0)
	 {
		 ret = -1;
		 goto exit;
	 }
