...
#define SPI_USE_ID                              SPI_ID_0
#define SPI_PAD_ACT                             GLB_SPI_PAD_ACT_AS_MASTER
#define SPI_USE_IRQ                             SPI_IRQn
...
static uint8_t spi_flash_mosi_gpio = GLB_GPIO_PIN_0;
...
static void spi_flash_gpio_init(void)
{
    ...
    GLB_GPIO_Func_Init(GPIO_FUN_SPI, (GLB_GPIO_Type *)gpioPins, sizeof(gpioPins)/sizeof(gpioPins[0]));
...
static void spi_flash_write_enable(void)
{
    ...
    cmd[0] = 0x06;
    SPI_Send_8bits(SPI_USE_ID,cmd,1,SPI_TIMEOUT_ENABLE);
    spi_flash_pull_cs_high();
}
...
static void spi_flash_wait_notbusy(void)
{
    ...
    while(1){
        ...
        SPI_SendRecv_8bits(SPI_USE_ID, (uint8_t *)wdata, (uint8_t *)rdata, 4, SPI_TIMEOUT_ENABLE);
        spi_flash_pull_cs_high();
...
static BL_Err_Type spi_flash_sector_erase(uint32_t secNum)
{
    ...
    stat = SPI_Send_8bits(SPI_USE_ID, (uint8_t *)wdata, 4, SPI_TIMEOUT_ENABLE);
    spi_flash_pull_cs_high();
...
static BL_Err_Type spi_flash_blk32_erase(uint32_t blkNum)
{
    ...
    // SPI_SetFrameSize(SPI_USE_ID, SPI_FRAME_SIZE_32);
    stat = SPI_Send_8bits(SPI_USE_ID, (uint8_t *)wdata, 1, SPI_TIMEOUT_ENABLE);
    spi_flash_pull_cs_high();
...
static BL_Err_Type spi_flash_page_write(uint32_t flashAddr, uint8_t *wBuff, uint32_t wLen)
{
    ...
    // SPI_SetFrameSize(SPI_USE_ID, SPI_FRAME_SIZE_8);
    SPI_Send_8bits(SPI_USE_ID, cmd, wLen+4, SPI_TIMEOUT_DISABLE);
    spi_flash_pull_cs_high();
...
static BL_Err_Type spi_flash_page_read(uint32_t flashAddr, uint8_t *rBuff, uint32_t rLen)
{
    ...
    // SPI_SetFrameSize(SPI_USE_ID, SPI_FRAME_SIZE_8);
    stat = SPI_SendRecv_8bits(SPI_USE_ID, (uint8_t *)cmd, (uint8_t *)resp, rLen+4, SPI_TIMEOUT_DISABLE);
    spi_flash_pull_cs_high();
    if(stat != SUCCESS){
        return stat;
    }
...
BL_Err_Type spi_flash_read_jedec_id(uint8_t *data)
{
    ...
    // SPI_SetFrameSize(SPI_USE_ID, SPI_FRAME_SIZE_32);
    spi_flash_pull_cs_low();
    stat = SPI_SendRecv_8bits(SPI_USE_ID, (uint8_t *)wdata, (uint8_t *)rdata, 4, SPI_TIMEOUT_DISABLE);
    if(stat != SUCCESS) {
        return stat;
    }
    spi_flash_pull_cs_high();
